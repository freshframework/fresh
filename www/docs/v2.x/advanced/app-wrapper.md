...
The app wrapper is the outermost component in Fresh's rendering hierarchy. It defines the `<html>`, `<head>`, and `<body>` tags that every page shares. It is only rendered on the server.
...
If you're using [file-based routing](/docs/concepts/file-routing), create a `routes/_app.tsx` file. Otherwise, register it programmatically with `app.appWrapper()`.
...
2. **[Layouts](/docs/concepts/layouts)** (`_layout.tsx`) - shared page chrome (nav, sidebar, footer)
...
The app wrapper receives the same props as page components - `url`, `state`, `params`, and more. This is useful for conditional logic:
...
Some routes may need to bypass the app wrapper entirely - for example, API routes that return JSON, or pages that need a completely different HTML structure. Use `skipAppWrapper` in the route config:
