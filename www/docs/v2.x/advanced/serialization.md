...
When Fresh renders a page on the server, [island](/docs/concepts/islands) props must be serialized to JSON and sent to the browser for hydration. Fresh uses a custom serialization system that supports more types than standard `JSON.stringify`.
...
2. **Client**: the value is wrapped in a new `signal()` call, creating a live reactive signal

If the same signal object is passed to multiple islands, it is serialized once and all islands receive the same signal instance on the client - keeping them synchronized.

Computed signals are serialized by reading their current value and wrapping it in `computed(() => value)` on the client. Since the original computation function cannot be transferred, the client-side computed signal holds a static value.
