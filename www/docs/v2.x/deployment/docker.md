...
> [info]: The `deno install --allow-scripts` step is required to populate `node_modules` and run any post-install scripts needed by npm packages (e.g. Tailwind CSS). This must happen before `deno task build`.
